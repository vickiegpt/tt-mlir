  ...
  LogicalResult matchAndRewrite(ToLayoutOp op,
    ...
    if (!inputL1) {
      ...
      if (inputLayout.isTiled()) {
        // Keep output layout but with input's element type
        auto bounceType = createModifiedType(
            rewriter.getContext(), outputType, outputLayout,
            /*memSpace=*/{}, /*grid=*/{}, inputType.getElementType());
        bounce(rewriter, op, bounceType);
      } else {
        assert(outputLayout.isTiled());
        // Keep input layout but with output's element type
        auto bounceType = createModifiedType(
            rewriter.getContext(), inputType, inputLayout,
            /*memSpace=*/{}, /*grid=*/{}, outputType.getElementType());
        bounce(rewriter, op, bounceType);
      }
      ...
      auto bounceType = createModifiedType(
          ...
          /*tileShape=*/ArrayRef<int64_t>{}); // Empty means no tiling
      bounce(rewriter, op, bounceType);
      ...
      auto bounceType =
          createModifiedType(rewriter.getContext(), outputType, outputLayout,
                             /*memSpace=*/{}, inputLayout.getGridShape());
      bounce(rewriter, op, bounceType);
    } else {
